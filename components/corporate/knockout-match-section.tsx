...
export function KnockoutMatchSection({
  ...
  const totalKnockout = STAGE_ORDER.reduce(
    (acc, stage) => acc + (grouped.get(stage)?.length ?? 0),
    0,
  );
  const totalPredicted = STAGE_ORDER.reduce((acc, stage) => {
    const stageMatches = grouped.get(stage) ?? [];
    return acc + stageMatches.filter((m) => predictions[m.id]).length;
  }, 0);
